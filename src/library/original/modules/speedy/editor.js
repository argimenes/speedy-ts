(function (factory) {
    ...
    function getRangeText(range) {
        ...
        whileNext(range.start, range.end, function (node) {
            ...
            if (node.speedy.stream == TEXT_STREAM.OUT || node.speedy.role != ELEMENT_ROLE.CHAR) {
                return;
            }
    ...
    function firstNextChar(start) {
        ...
        var char = nextUntil(start, node => {
            ...
            return node.speedy && node.speedy.role == ELEMENT_ROLE.CHAR && node.speedy.stream == TEXT_STREAM.IN && !node.speedy.isLineBreak;
        });
        return char;
    }
    ...
    var Editor = (function () {
        ...
        Editor.prototype.setupEventHandlers = function () {
            ...
            document.body.addEventListener("keydown", function (e) {
                ...
                var tmp = _this.getCurrentTEST();
                var current = tmp.node;
                _this.processControlOrMeta({ event: e, current })
            });
                ...
                props.filter(x => !!x.schema.event && !!x.schema.event.annotation)
                    .forEach(x => {
                        ...
                        for (var key in annotation) {
                            ...
                            if (key == "control-shift-click" && (false == e.ctrlKey || false == e.shiftKey)) {
                                continue;
                            }
                            if (key == "alt-click" && false == e.altKey) {
                                continue;
                            }
                            if (key == "meta-click" && false == e.metaKey) {
                                continue;
                            }
                            if (key == "control-click" && false == e.ctrlKey) {
                                continue;
                            }
                            if (key == "shift-click" && false == e.shiftKey) {
                                continue;
                            }
                            annotation[key](x);
                        }
        ...
        Editor.prototype.handleDoubleClickEvent = function (e) {
            ...
            var props = this.data.properties.filter(function (prop) {
                ...
                return schema && schema.event && schema.event.annotation && schema.event.annotation["dblclick"] && isWithin(prop.startNode, prop.endNode, e.target);
            });
            if (!props.length) {
                return;
            }
        ...
        Editor.prototype.processControlOrMeta = function (data) {
            ...
            var control = data.event.ctrlKey && data.event.key != "Control", shift = data.event.shiftKey && data.event.key != "Shift";
            var processed = false;
            if (control && false == shift) {
                ...
                if (keyboard) {
                    let handlerName = "control-" + data.event.key.toUpperCase();
                    if (keyboard[handlerName]) {
                        keyboard[handlerName]({ properties: propsAtCaret, editor: this });
                        processed = true;
                    }
                }
            }
        ...
        Editor.prototype.insertCharacter = function (data) {
            ...
            this.buffer.values.push({ event: data.event, current: span });
            if (this.buffer.values.length > this.buffer.maxLength) {
                this.buffer.values.splice(0, 1);
            }
            console.log({ buffer: this.buffer.values });
        ...
        Editor.prototype.clearSelection = function (start, end) {
        ...
        Editor.prototype.getParentSpan = function (node) {
            ...
            while (node.speedy && node.speedy.role != ELEMENT_ROLE.CHAR) {
        ...
        Editor.prototype.createBlockProperty = function (propertyTypeName, propertyRange) {
            var range = propertyRange || this.getSelectionNodes();
            this.createBlock(range.start, range.end, propertyTypeName);
            var property = new Property({
                editor: this,
        ...
        Editor.prototype.createMetadataProperty = function (args) {
        ...
        Editor.prototype.bind = function (model) {
            ...
            this.setAnimationFrame();
            if (!model.text || !model.text.length) {
        ...
        Editor.prototype.getSentences = function () {
        ...
        Editor.prototype.getTextBlocks = function () {
            var spans = this.mark().filter(x => x.speedy.role == ELEMENT_ROLE.CHAR && x.speedy.stream == TEXT_STREAM.IN);
            var text = spans.map(x => x.textContent).join("");
            var re = new RegExp(/[\r]+[.\r]+[\])]*|.+/, "g");
            var results = [];
