(function (factory) {
    ...
    var Editor = (function () {
        function Editor(cons) {
            ...
            this.onContextMenuActivated = cons.onContextMenuActivated;
            this.onContextMenuDeactivated = cons.onContextMenuDeactivated;
        ...
        Editor.prototype.setupEventHandlers = function () {
            ...
            this.container.addEventListener("contextmenu", e => {
                ...
                if (_this.onContextMenuActivated) {
                    var range = this.getSelectionNodes();
                    _this.onContextMenuActivated({ editor: _this, e, range });
                }
                return false;
            });
            window.addEventListener("click", e => {
                if (_this.mode.contextMenu.active) {
                    if (_this.onContextMenuDeactivated) {
                        _this.onContextMenuDeactivated({ editor: _this, e });
                    }
                }
            });
        };
        ...
        Editor.prototype.getCurrentTEST = function () {
            var sel = window.getSelection();
            // var current = sel.anchorNode.parentElement;
            if (sel.anchorNode.speedy && sel.anchorNode.speedy.role == ELEMENT_ROLE.ROOT) {
                return {
                    node: this.createSpan(),
                    offset: 0
                };
            }
        ...
        Editor.prototype.processControlOrMeta = function (data) {
            ...
            var props = this.getCurrentRanges(data.current).filter(x => x.schema.event && x.schema.event.keyboard);
            var control = event.ctrlKey, shift = event.shiftKey;
            if (control && shift) {
                props.forEach(cp => {
                    var keyboard = cp.schema.event.keyboard;
                    var handlerName = "control-shift-" + data.event.key.toUpperCase();
        ...
        Editor.prototype.handleKeyDownEvent = function (evt) {
            var _ = this;
            var canEdit = !!!this.lockText;
            var tmp = this.getCurrentTEST();
            var current = tmp.node; // this.getCurrent();
            ...
            if (evt.ctrlKey || evt.metaKey || evt.keyCode == ESCAPE) {
                var result = this.processControlOrMeta({ event: evt, current });
